...
<head>
  ...
  <title>MenuAI | Version SaaS Premium (App Mobile)</title>
  ...
  <style>
    ...
    *{margin:0;padding:0;box-sizing:border-box;}
    ...
    header{position:relative;z-index:1;text-align:center;padding:1rem 1rem 0;border-bottom:1px solid var(--gold-light);}
    ...
    .tabs{display:flex;justify-content:center;max-width:460px;margin:1rem auto 0;border:1px solid var(--gold-light);border-bottom:none;border-radius:2px 2px 0 0;overflow:hidden;}
    ...
    main{position:relative;z-index:1;max-width:1200px;margin:0 auto;padding:2rem 1.5rem;display:grid;grid-template-columns:1fr 1fr;gap:2rem;align-items:start;}
    @media(max-width:768px){main{grid-template-columns:1fr;gap:2rem;}}
    .panel{background:var(--card-bg);border:1px solid var(--gold-light);border-radius:2px;padding:2rem;position:relative;}
    ...
    .panel-subtitle{font-family:'DM Mono',monospace;font-size:0.65rem;color:var(--warm-gray);letter-spacing:0.2em;text-transform:uppercase;margin-bottom:1.5rem;}
    .tab-content{display:none;} .tab-content.active{display:block;}
    .photo-drop-zone{border:2px dashed var(--gold-light);border-radius:2px;padding:1.5rem 1.5rem;text-align:center;cursor:pointer;transition:all 0.3s;position:relative;}
    ...
    #progress-box{display:none;margin-top:1rem;}
    ...
    .item-tags { font-size: 0.9rem; margin-left: 8px; user-select: none; }

    /* --- BASE CARTE MENU --- */
    .menu-card { transition: all 0.3s ease; background: var(--card-bg); color: var(--ink); border: 1px solid var(--gold-light); border-radius: 2px; position: relative; padding-bottom: 2rem; font-family: 'Cormorant Garamond', Georgia, serif; padding-top: 1rem;}
    ...
    .menu-header { background: var(--ink); color: var(--cream); padding: 2rem; text-align: center; position: relative; transition: all 0.3s ease; }
    ...
    .menu-section { padding: 1rem 2rem 0.5rem; }
    .section-title { font-size: 1rem; font-weight: 600; letter-spacing: 0.15em; color: var(--rust); text-transform: uppercase; margin-bottom: 0.8rem; border-bottom: 1px solid var(--gold-light); transition: all 0.3s ease; }
    .menu-item { display: flex; justify-content: space-between; align-items: baseline; gap: 1rem; padding: 0.5rem 0; border-bottom: 1px dotted rgba(201,168,76,0.2); transition: all 0.3s ease; }
    .item-name-block { display: flex; align-items: center; }
    .item-name { font-size: 1rem; font-weight: 500; color: inherit; display: inline-block; }
    .item-desc { font-size: 0.8rem; font-style: italic; color: var(--warm-gray); margin-top: 0.1rem; }
    .item-price { font-family: 'DM Mono', monospace; font-size: 0.85rem; color: var(--gold); font-weight: bold; white-space: nowrap; }
    .menu-footer { margin-top: 1.5rem; padding: 1rem; text-align: center; border-top: 1px solid var(--gold-light); font-family: 'DM Mono', monospace; font-size: 0.6rem; color: var(--warm-gray); transition: all 0.3s ease; }

    /* =========================================
       THÈME SUR-MESURE (GÉNÉRÉ PAR IA) 
    ========================================= */
    .menu-card.theme-custom { background-color: var(--ai-bg, #ffffff); color: var(--ai-text, #111111); border: 2px solid var(--ai-primary, #333333); font-family: var(--ai-font, 'Cormorant Garamond', serif); }
    .menu-card.theme-custom::before { border: 1px solid var(--ai-accent, #cccccc); top: 6px; left: 6px; right: -6px; bottom: -6px; }
    .menu-card.theme-custom .menu-header { background-color: var(--ai-primary, #333333); color: var(--ai-bg, #ffffff); border-bottom: 3px solid var(--ai-accent, #cccccc); }
    .menu-card.theme-custom .section-title { color: var(--ai-primary, #333333); border-bottom: 2px dashed var(--ai-accent, #cccccc); }
    .menu-card.theme-custom .item-name { color: var(--ai-text, #111111); }
    .menu-card.theme-custom .item-desc { color: var(--ai-text, #555555); opacity: 0.8; }
    .menu-card.theme-custom .item-price { color: var(--ai-primary, #333333); background: var(--ai-bg, transparent); padding: 0 4px; }
    .menu-card.theme-custom .menu-item { border-bottom: 1px dotted var(--ai-accent, #cccccc); }
    .menu-card.theme-custom .menu-footer { border-top: 1px solid var(--ai-accent, #cccccc); color: var(--ai-text, #111111); opacity: 0.7; }
    ...
    .menu-card.theme-minimal .menu-header { background: #ffffff; color: #111111; border-bottom: 2px solid #111111; padding: 2rem 1.5rem; }
    ...
    .menu-card.theme-fastfood .menu-header { background: #E53935; color: #FFEB3B; padding: 1.5rem; }
    .menu-card.theme-fastfood .section-title { color: #E53935; border-bottom: 3px solid #FFEB3B; font-size: 1.2rem; }
    .menu-card.theme-fastfood .item-name { font-family: Arial, sans-serif; font-weight: 900; font-size: 1rem; text-transform: uppercase; }
    ...
    .menu-card.theme-french .menu-header { background: #1A237E; color: #ffffff; padding-bottom: 2rem; }
    ...
    .deploy-box p { font-family: 'DM Mono', monospace; font-size: 0.75rem; color: var(--warm-gray); margin-bottom: 1rem; line-height: 1.4; }
    .deploy-inputs { display: flex; gap: 0.5rem; justify-content: center; margin-bottom: 1rem; flex-wrap: wrap; }
    ...
    #qrcode-container { margin-top: 1.5rem; display: none; flex-direction: column; align-items: center; gap: 1rem; background: white; padding: 1.5rem; border: 1px solid var(--gold-light); }
    ...
    /* --- PANNEAU DE CONFIGURATION DE BIENVENUE --- */
    #config-modal { display: none; position: fixed; inset: 0; background: rgba(26,20,16,0.95); z-index: 1000; align-items: center; justify-content: center; padding: 2rem; }
    #config-modal.active { display: flex; }
    .config-panel { background: var(--card-bg); border: 2px solid var(--gold); border-radius: 4px; padding: 3rem; max-width: 500px; text-align: center; box-shadow: 0 10px 50px rgba(0,0,0,0.5); }
    .config-logo { font-size: 3rem; margin-bottom: 1rem; color: var(--gold); }
    .config-title { font-size: 1.8rem; font-weight: 300; color: var(--ink); margin-bottom: 0.5rem; }
    .config-title span { color: var(--gold); font-style: italic; }
    .config-subtitle { font-family: 'DM Mono', monospace; font-size: 0.75rem; color: var(--warm-gray); text-transform: uppercase; margin-bottom: 2rem; }
    .config-group { margin-bottom: 1.5rem; text-align: left; }
    .config-label { font-family: 'DM Mono', monospace; font-size: 0.65rem; color: var(--gold); text-transform: uppercase; margin-bottom: 0.5rem; display: block; }
    
    /* MODIFICATION ICI : On utilise type="password" pour cacher les clés */
    .config-input { width: 100%; padding: 0.8rem 1rem; border: 1px solid var(--gold-light); border-radius: 2px; background: white; font-family: 'DM Mono', monospace; font-size: 1rem; outline: none; }
    
    .config-hint { font-size: 0.8rem; font-style: italic; color: var(--warm-gray); margin-top: 0.4rem; line-height: 1.3; }
    .btn-save-config { display: block; width: 100%; padding: 1rem; background: var(--gold); color: var(--ink); border: none; font-family: 'DM Mono', monospace; font-size: 0.8rem; font-weight: bold; text-transform: uppercase; cursor: pointer; transition: 0.2s; border-radius: 2px; }
    .btn-save-config:hover { background: var(--gold-light); }
  </style>
</head>
<body>
...
<div id="config-modal">
  <div class="config-panel">
    ...
    <div class="config-title">Bienvenue sur Menu<span>AI</span></div>
    <div class="config-subtitle">Configuration de vos clés professionnelles</div>
    
    <div class="config-group">
      <label class="config-label">Clé API Gemini (IA)</label>
      <input type="password" class="config-input" id="setup-gemini-key" placeholder="AIzaSy..."/>
      <p class="config-hint">Requise pour l'analyse de photos et la traduction.</p>
    </div>
    
    <div class="config-group">
      <label class="config-label">Token GitHub Classique (ghp_...)</label>
      <input type="password" class="config-input" id="setup-gh-token" placeholder="ghp_..."/>
      <p class="config-hint">Requis pour publier les menus et générer le QR Code. Le token doit avoir les droits 'repo'.</p>
    </div>
    
    <button class="btn-save-config" onclick="saveConfiguration()">Enregistrer et commencer</button>
  </div>
</div>
...
<header>
  ...
  <p class="tagline">Génération SaaS (IA, Traduction, Tags Intelligents)</p>
  <div class="tabs">
...
<main>
  <div>
    <div class="panel">
      <div class="tab-content active" id="tab-photo">
        ...
        <div class="panel-subtitle">Extraction, Traduction & Détection Allergènes</div>
        ...
        <button class="btn-generate-full" id="btn-analyze" onclick="analyzePhotos()">Générer la carte complète</button>
      </div>
      ...
      <div id="progress-box">
        ...
        <div class="progress-step" id="step-2"><div class="step-dot"></div><span id="lbl-2">Traduction et détection des tags en cours...</span></div>
        <div class="progress-step" id="step-3"><div class="step-dot"></div><span id="lbl-3">Structuration de la carte</span></div>
        <div class="progress-step" id="step-4"><div class="step-dot"></div><span id="lbl-4">Application du thème</span></div>
      </div>
    </div>
  </div>
  ...
  <div id="output-panel">
    <div class="edit-hint">
      ✏️ Clic direct pour modifier. Testez les boutons FR/EN ci-dessous pour voir la traduction !
    </div>

    <div class="style-switcher">
      <label for="theme-select">🎨 Style du menu :</label>
      <select id="theme-select" onchange="changeTheme()">
        <option value="theme-custom">✨ 100% Sur-mesure (Créé par IA)</option>
    ...
    <div class="menu-card theme-custom" id="menu-card"></div>
    ...
    <div class="deploy-box">
      ...
      <p>Crée automatiquement un site web gratuit via GitHub Pages.</p>
  ...
  <div class="settings-btn">
    <button onclick="resetConfiguration()">⚙️ Paramètres : Changer les clés API</button>
  </div>
</main>
...
function saveConfiguration() {
  ...
  const github = document.getElementById('setup-gh-token').value.trim();
  
  if (!gemini || !github) {
    alert("Les deux clés sont obligatoires pour que l'application fonctionne.");
    return;
  }
  ...
  alert("C'est parfait ! Vos clés sont enregistrées en toute sécurité dans votre téléphone.");
}

function resetConfiguration() {
  const confirmReset = confirm("Voulez-vous vraiment effacer vos clés API et token GitHub de ce téléphone ?");
  if (confirmReset) {
    localStorage.removeItem("geminiKey");
    localStorage.removeItem("githubToken");
    alert("Les clés ont été effacées. La page va se recharger pour vous demander les nouvelles clés.");
    location.reload();
  }
}
...
// --- APPEL API GEMINI (PROMPT INTELLIGENT POUR SÉPARER LES LANGUES) ---
async function callGemini(parts, restoInfo) {
  if (!GEMINI_API_KEY) {
    alert("Oups, la clé Gemini est manquante. Allez dans Paramètres.");
    throw new Error("Clé API manquante.");
  }

  const systemPrompt = `Tu es un expert en numérisation de menus, traducteur et designer web.
  INSTRUCTIONS:
  1. Extrais TOUS les plats et prix de l'image.
     ⚠️ ATTENTION : Si le menu sur la photo est DÉJÀ bilingue et que les mots sont collés (ex: "Poulet TikkaChicken Tikka" ou "Naan Nature ou Cheese Plain or Cheese Naan"), tu DOIS absolument séparer le français de l'anglais ! Ne les laisse pas collés.
  2. TRADUCTION & NETTOYAGE : 
     - Mets UNIQUEMENT le texte en Français (nettoyé) dans les clés "title", "name", et "desc".
     - Mets UNIQUEMENT le texte en Anglais (soit traduit, soit extrait proprement) dans les clés "title_en", "name_en", et "desc_en".
  3. TAGS : 
     - Ajoute "🌱" UNIQUEMENT pour les Entrées et Plats principaux qui ne contiennent ni viande ni poisson. 
     - ⚠️ RÈGLE STRICTE : Ne mets JAMAIS le tag "🌱" sur les Desserts ni sur les Boissons.
     - Ajoute "🌶️" si le plat est épicé.
     - Mets un tableau vide [] par défaut.
  4. DESIGN : Analyse l'image et fournis un design adapté (codes couleurs HEX).
  ...
  STRUCTURE JSON:
  {
    ...
    "tagline_en": "Inspired Cuisine • Homemade",
    "address": "${restoInfo.addr || ''}",
    "design": {
      "backgroundColor": "#HEX", "textColor": "#HEX", "primaryColor": "#HEX", "accentColor": "#HEX", "fontStyle": "serif"
    },
    "sections": [
      { 
        "title": "Entrées", "title_en": "Starters", 
        "items": [
          { "name": "Poulet Tikka", "name_en": "Chicken Tikka", "desc": "Desc FR", "desc_en": "Desc EN", "price": "6,90€", "tags": ["🌶️"] }
        ] 
      }
    ]
  }`;
  ...
  if (!resp.ok) {
    ...
    if (errorDetails.error?.code === 404) {
        ...
        throw new Error(errorDetails.error?.message || "Erreur API.");
    }
  }
...
function renderMenu(data, elapsed) {
  ...
  if (data.design) {
    card.style.setProperty('--ai-bg', data.design.backgroundColor || '#ffffff');
    card.style.setProperty('--ai-text', data.design.textColor || '#111111');
    card.style.setProperty('--ai-primary', data.design.primaryColor || '#333333');
    card.style.setProperty('--ai-accent', data.design.accentColor || '#cccccc');
    let font = "'Cormorant Garamond', serif";
    if(data.design.fontStyle === 'sans-serif') font = "Arial, Helvetica, sans-serif";
    if(data.design.fontStyle === 'cursive') font = "'Brush Script MT', cursive, serif";
    card.style.setProperty('--ai-font', font);
  }
  ...
  card.innerHTML = `
    ...
    <div class="menu-header">
      ...
      <div class="menu-tagline-text lang-fr" contenteditable="true">${data.tagline}</div>
      <div class="menu-tagline-text lang-en" contenteditable="true">${data.tagline_en || data.tagline}</div>
    </div>
    
    ${data.sections.map(s => `
      <div class="menu-section">
        <div class="section-title lang-fr" contenteditable="true">${s.title}</div>
        <div class="section-title lang-en" contenteditable="true">${s.title_en || s.title}</div>
        
        ${s.items.map(it => `
          <div class="menu-item">
            <div style="flex:1;">
              <div class="item-name-block">
                <span class="item-name lang-fr" contenteditable="true">${it.name}</span>
                <span class="item-name lang-en" contenteditable="true">${it.name_en || it.name}</span>
                <span class="item-tags">${(it.tags || []).join('')}</span>
              </div>
              <div class="item-desc lang-fr" contenteditable="true">${it.desc || ''}</div>
              <div class="item-desc lang-en" contenteditable="true">${it.desc_en || ''}</div>
            </div>
...
function hideProgress() {
  setTimeout(() => document.getElementById('progress-box').style.display = 'none', 800);
}

async function advanceStep(n, d) {
  if (currentStep >= 0) {
    document.getElementById('step-' + (currentStep + 1)).className = 'progress-step done';
  }
  currentStep = n - 1;
  ...
  r.onload = ev => {
    ...
    img.onload = () => {
      ...
      if (code) {
        ...
        document.getElementById('qr-url-display').textContent = "Lien : " + code.data;
      } else {
        showError("QR Code illisible.");
      }
    };
...
async function deployAndGenerateQR() {
  ...
  if (!GITHUB_TOKEN || !repoName) {
    alert("Le Token GitHub est manquant ( Paramètres) ou le nom du lien est vide.");
    return showError("Token GitHub ou Nom du lien obligatoire.");
  }
  ...
  try {
    let res = await fetch('https://api.github.com/user', { headers: { 'Authorization': `token ${GITHUB_TOKEN}` } });
    if (!res.ok) throw new Error("Le Token GitHub semble invalide ou expiré.");
    ...
    const styles = document.querySelector('style').innerHTML; 
    const fullHtml = `<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no"><title>Menu | ${username}</title><link href="https://fonts.googleapis.com/css2?family=Cormorant+Garamond:ital,wght@0,300;0,400;0,600;1,300;1,400&family=DM+Mono:wght@300;400;500&display=swap" rel="stylesheet"/><style>${styles} body { background: #f5f0e8; padding: 10px; } .menu-card { max-width: 800px; margin: auto; box-shadow: 0 5px 20px rgba(0,0,0,0.1); }</style></head><body>${content}</body></html>`;
    ...
    let fileSha = null;
    const getRes = await fetch(putUrl, { headers: { 'Authorization': `token ${GITHUB_TOKEN}` } });
    if (getRes.ok) {
      const fileData = await getRes.json();
      fileSha = fileData.sha;
    }

    const body = { message: "MenuAI Pro Deploy (Multilingue & Tags)", content: encodedHtml };
    ...
    await fetch(`https://api.github.com/repos/${username}/${repoName}/pages`, {
      method: 'POST',
      headers: { 'Authorization': `token ${GITHUB_TOKEN}`, 'Accept': 'application/vnd.github.switcheroo-preview+json' },
  ...
  if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => {
      navigator.serviceWorker.register('./sw.js')
        .then(reg => console.log('Appli installée en cache !', reg.scope))
